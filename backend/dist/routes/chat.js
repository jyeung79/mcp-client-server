...
export function createChatRouter(mcpClient) {
    const router = Router();
    router.post("/", async (req, res) => {
        ...
        const apiKey = aiProvider === "openai" ? config.openaiApiKey : config.anthropicApiKey;
        if (!apiKey) {
            res.status(400).json({
                error: `Missing API key for provider \"${aiProvider}\". Set ${aiProvider === "openai" ? "OPENAI_API_KEY" : "ANTHROPIC_API_KEY"}.`,
            });
            return;
        }
        ...
        try {
            ...
            for (let i = 0; i < MAX_ITERATIONS; i++) {
                const result = await adapter.streamChat(conversation, tools, (text) => {
